program param
  ...
  real, dimension(:), allocatable :: xu, yu, xl, yl, xf, yf, bump_pos, xunew, yunew, xlnew, ylnew, xfnew, yfnew, derivativeu(:,:)
  ...
  real (kind = 8) :: xref, yref,firstterm, secondterm, areaold, areanew, buffer
...
do i = 1,nodu
  ...
  do j = 1,ndp
      ...
      derivativeu(i,j) = (sin(pi*xunew(i)**m)**t_b)
  enddo
  yunew(i) = yunew(i) + sum
...
do i = 1,nodl
  ...
  do j = 1,ndp
      ...
      derivativel(i,ndp+j) = (sin(pi*xlnew(i)**m)**t_b)
  enddo
  ylnew(i) = ylnew(i) + sum
...
enddo
areaold = 0.5*abs(firstterm - secondterm + (x(nodu+nodl)*y(1)) - (x(1)*y(nodu+nodl)))
...
do i = 1,nodu+nodl-1
  ...
  secondterm = secondterm + x(i+1)*y(i)
enddo
areanew = 0.5*abs(firstterm - secondterm + (x(nodu+nodl)*y(1)) - (x(1)*y(nodu+nodl)))
...
do i = 1,(2*ndp)+3
  ...
  enddo
  write(2,*)sum + gamma*(areaold-0.5)*areagrad(i)
  rewind(1)
enddo
write(2,*) 'END'
close(1)
