program gfn
  ...
  integer :: flagupper, flaglower,flagflap, pos, ierr, pair(2), nbe, i, j ,k,l, counter, flag, nodu, nodl, nodf
  integer, dimension(:), allocatable :: boudata(:,:), geoboudata(:,:), boudataupper(:,:), boudatalower(:,:), boudataflap(:,:), unodes, lnodes,fnodes
...
k = 1
do i = 1, nodf
  fnodes(k) = boudataflap(i,2)
  fnodes(k+1) = boudataflap(i,3)
  fnodes(k+2) = boudataflap(i,4)
  k = k + 3
enddo
